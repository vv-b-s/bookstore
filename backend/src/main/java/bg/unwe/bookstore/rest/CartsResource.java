...
public class CartsResource {
    ...
    @Path("/{id}/Items")
    @Produces(APPLICATION_JSON)
    public Response addItemToCart(@PathParam("id") int cartId, @QueryParam("bookId") Integer bookId) {
        Optional<Cart> cart = cartRepository.findById(cartId);
        Optional<Book> bookDB = bookRepository.findById(bookId);

        if (cart.isPresent() && bookDB.isPresent()) {
            CartBook cartBook = cartBookRepository.persist(new CartBook(cart.get(), bookDB.get(), LocalDateTime.now(), 1));
    ...
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    public Response editCart(Cart cart) {
    ...
    @Path("/{id}/Items")
    @Transactional
    public Response removeItemFromCart(@PathParam("id") int cartId, @QueryParam("bookId") Integer bookId) {
        cartBookRepository.removeByCartAndBookId(cartId, bookId);
